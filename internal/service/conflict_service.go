    ...
    for _, group := range grouped {
        for i := 0; i < len(group); i++ {
            for j := i + 1; j < len(group); j++ {
                if c := s.detectConflict(&group[i], &group[j]); c != nil {
...
func (s *ConflictService) detectConflict(a, b *domain.Override) *domain.Conflict {
    ...
    if a.Specificity != b.Specificity { return nil }

    // Must have overlapping effective date ranges
    // Handle pointer types for ExpiresDate
    end1 := time.Time{}
    end2 := time.Time{}
    if a.ExpiresDate != nil { end1 = *a.ExpiresDate }
    if b.ExpiresDate != nil { end2 = *b.ExpiresDate }
    
    if !s.dateRangesOverlap(a.EffectiveDate, end1, b.EffectiveDate, end2) {
        return nil
    }
...
func (s *ConflictService) buildConflictReason(o *domain.Override) string {
    ...
    return "Same step/trait, specificity " + string(rune(o.Specificity+48)) + ", overlapping dates, compatible selectors " + selectors
}
